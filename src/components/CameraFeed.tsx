...
interface CameraFeedProps {
  ...
  isAlert: boolean;
  cameraStream?: MediaStream | null; // 🔥 NEW PROP
}

const CameraFeed = ({ cameras, isAlert, cameraStream }: CameraFeedProps) => {
      ...
      {mainCamera && (
        <div
          className={`relative aspect-video rounded bg-card border overflow-hidden transition-shadow duration-300 ${
            isAlert
              ? "glow-danger border-danger/40"
              : mainCamera.suspiciousActivity
              ? "glow-warning border-warning/40"
              : "border-border"
          }`}
        >
          {/* ✅ SHOW REAL CAMERA IF AVAILABLE */}
          {cameraStream ? (
            <video
              ref={videoRef}
              autoPlay
